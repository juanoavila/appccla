...
class ModificarPagoPorDepositoViewController: UIViewController {
        ...
        if(!CheckConnection.Connection()){
            ...
            if(!validarFormulario()){
                ...
                if(self.responsePOST.status == "success"){
                    let alert = UIAlertController(title: "¡Bien!", message: "Tu solicitud fue modificada exitosamente", preferredStyle: UIAlertController.Style.alert)
    ...
    func obtenerTiposCuentas(){
        ...
        let urlString = "http://mfpqa-cajalosandes.mybluemix.net/mfp/api/adapters/BancosTipoCuenta/?flag=1"
        guard let url = URL(string: urlString) else { return}
        let session = URLSession.shared
        var request = URLRequest(url: url)
        request.httpMethod = "GET"
